...
def batch_norm(x, scope, train_phase=True):
    bn_train = batch_norm_layer(x, decay=0.999, center=True, scale=True, updates_collections=None, is_training=True, reuse=None,  # is this right?
                          trainable=True, scope=scope)
    #bn_inference = batch_norm_layer(x, decay=0.999, center=True, scale=True, updates_collections=None, is_training=False, reuse=True,  # is this right?
    #                          trainable=True, scope=scope)
    #z = tf.cond(train_phase, lambda: bn_train, lambda: bn_inference)
    return bn_train

class batch_norm1(object):
    ...
    def __call__(self, x, train=True):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            out = tf.nn.batch_norm_with_global_normalization(x, self.mean, self.variance, self.beta, self.gamma, self.epsilon, scale_after_normalization=True)
            if needs_reshape:
