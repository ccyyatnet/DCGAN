...
class DCGAN(object):
    ...
    def train(self, config):
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                batch_images = data.load_data_RGB(config, idx)
                ...
                if np.mod(counter, 200) == 1:
                    save_size = int(math.sqrt(config.batch_size))
                    #z1
                    _generate_image, _loss, _prob_real, _prob_fake = self.sess.run([self.generate_images_RGB, self.total_loss, self.avg_prob_real, self.avg_prob_fake], feed_dict={self.z: sample_z1, self.images_RGB: sample_images})
                    ...
                    #z2
                    _generate_image, _loss, _prob_real, _prob_fake = self.sess.run([self.generate_images_RGB, self.total_loss, self.avg_prob_real, self.avg_prob_fake], feed_dict={self.z: sample_z2, self.images_RGB: sample_images})
                    ...
                    #z3
                    _generate_image, _loss, _prob_real, _prob_fake = self.sess.run([self.generate_images_RGB, self.total_loss, self.avg_prob_real, self.avg_prob_fake], feed_dict={self.z: sample_z3, self.images_RGB: sample_images})
    ...
    def discriminator(self, image, y=None, reuse=False, config=None):
        ...
        h0 = lrelu(conv2d(image, config.df_dim, k_h=config.d_kernel_size, k_w=config.d_kernel_size, name='d_h0_conv'))
        h1 = lrelu(batch_norm(conv2d(h0, config.df_dim * 2, k_h=config.d_kernel_size, k_w=config.d_kernel_size, name='d_h1_conv'), 'd_bn1'))
        h2 = lrelu(batch_norm(conv2d(h1, config.df_dim * 4, k_h=config.d_kernel_size, k_w=config.d_kernel_size, name='d_h2_conv'), 'd_bn2'))
        h3 = lrelu(batch_norm(conv2d(h2, config.df_dim * 8, k_h=config.d_kernel_size, k_w=config.d_kernel_size, name='d_h3_conv'), 'd_bn3'))
    ...
    def test_random_z(self, config=None):
        ...
        for test_idx, test_image_idx in enumerate(test_idxs[:config.number_of_test_images]):
            ...
            test_image_batch = np.array([test_image for i in range(config.batch_size)], dtype=np.float32)
            print test_image_batch.shape, test_image_batch.max(), test_image_batch.dtype

            test_the_image_batch = data.load_the_data(config, 0)
            print test_the_image_batch.shape, test_the_image_batch.max(), test_the_image_batch.dtype
